...
from sso import delete_account_assigment
...
def lambda_handler(event, context):
    ...
    if POST_UPDATE_TO_SLACK and TOKEN == '':
        error = f'POST_UPDATE_TO_SLACK is set and thus TOKEN is required but it is empty'
        logger.error(error)
        raise Exception(error)
        ...
        for permision_set in config_lookup('permission_sets'):
            ...
            for account_assigment in response['AccountAssignments']:
                ...
                if account_assigment['PrincipalType'] == 'GROUP':
                    # skip groups
                    continue
                role_name = config_lookup('permission_sets', 'arn', account_assigment['PermissionSetArn'], 'name')
                email = config_lookup('users', 'sso_id', account_assigment['PrincipalId'], 'email')
                slack_id = config_lookup('users', 'sso_id', account_assigment['PrincipalId'], 'slack_id')
