...
class CacheKey:
    ...
    USERS_WITH_ATTRIBUTES = "users_with_attributes.json"
    GROUPS = "groups.json"
...
# -----------------User and Group Caching for Attribute Sync-----------------#


def get_cached_users_with_attributes(
    s3_client: S3Client,
    cache_config: CacheConfig,
) -> Optional[list[dict]]:
    """Get cached users with attributes from S3.
    ...
    Returns:
        List of cached user dictionaries or None if cache miss
    """
    if not cache_config.enabled:
        logger.debug("Cache is disabled, skipping cache lookup for users")
        return None
    ...
    try:
        ...
        response = s3_client.get_object(
            Bucket=validated_bucket_name,
            Key=CacheKey.USERS_WITH_ATTRIBUTES,
        )

        users_data = json.loads(response["Body"].read().decode("utf-8"))
        logger.info(f"Retrieved {len(users_data)} users with attributes from cache")
        return users_data

    except s3_client.exceptions.NoSuchKey:
        logger.info("Cache miss for users with attributes - no cached data found")
        return None
    except Exception as e:
        logger.warning(f"Failed to get cached users with attributes: {e}", extra={"error": str(e)})
        return None


def set_cached_users_with_attributes(
    s3_client: S3Client,
    cache_config: CacheConfig,
    users: list[dict],
) -> None:
    """Store users with attributes in S3 cache.

    Args:
        s3_client: S3 client
        cache_config: Cache configuration
        users: List of user dictionaries to cache
    """
    if not cache_config.enabled:
        logger.debug("Cache is disabled, skipping cache write for users")
        return
    ...
    try:
        ...
        logger.info(f"Cached {len(users)} users with attributes")

    except ValueError as e:
        logger.warning(f"Validation failed when caching users: {e}", extra={"error": str(e)})
    except Exception as e:
        logger.warning(f"Failed to cache users with attributes: {e}", extra={"error": str(e)})
...
def get_cached_groups(
    ...
    """Get cached groups (name to ID mapping) from S3.
    ...
    Returns:
        Dictionary mapping group names to IDs, or None if cache miss
    """
    if not cache_config.enabled:
        logger.debug("Cache is disabled, skipping cache lookup for groups")
        return None
    ...
    try:
        ...
        logger.info(f"Retrieved {len(groups_data)} groups from cache")
        return groups_data

    except s3_client.exceptions.NoSuchKey:
        logger.info("Cache miss for groups - no cached data found")
        return None
    except Exception as e:
        logger.warning(f"Failed to get cached groups: {e}", extra={"error": str(e)})
        return None


def set_cached_groups(
    s3_client: S3Client,
    cache_config: CacheConfig,
    groups: dict[str, str],
) -> None:
    """Store groups (name to ID mapping) in S3 cache.
    ...
    """
    if not cache_config.enabled:
        logger.debug("Cache is disabled, skipping cache write for groups")
        return
    ...
    try:
        ...
        logger.warning(f"Validation failed when caching groups: {e}", extra={"error": str(e)})
    except Exception as e:
        logger.warning(f"Failed to cache groups: {e}", extra={"error": str(e)})
