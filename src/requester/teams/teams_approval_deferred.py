...
GROUP_APPROVAL_INTERNAL_ACTION = "teams_post_group_approval"
...
def sign_group_approval_post(
    ...
    pa = (teams.parent_activity_id or "").strip()
    lid = (teams.launcher_activity_id or "").strip()
    body = f"v5|group|{elevator_id}|{requester_email.lower()}|{cid}|{su}|{pa}|{lid}"
    return hmac.new(_hmac_key(), body.encode(), hashlib.sha256).hexdigest()


def verify_group_approval_post(
    elevator_id: str,
    requester_email: str,
    teams: AccountApprovalTeamsThread,
    signature: str,
    ...
    try:
        expect = sign_group_approval_post(elevator_id, requester_email, teams)
    except ValueError:
        return False
    return hmac.compare_digest(expect, signature)


def invoke_group_approval_post_async(
    elevator_id: str,
    requester_display_name: str,
    requester_email: str,
    teams: AccountApprovalTeamsThread,
    ...
    cid = (teams.conversation_id or "").strip()
    pay: dict[str, Any] = {
        "internal_action": GROUP_APPROVAL_INTERNAL_ACTION,
        "elevator_id": elevator_id,
        "requester_display_name": requester_display_name,
        "requester_email": requester_email,
        "teams_conversation_id": cid,
        ...
        "teams_parent_activity_id": tpar,
        "teams_launcher_activity_id": tlaunch,
        "hmac": sign_group_approval_post(elevator_id, requester_email, teams),
    }
    boto3.client("lambda").invoke(
        FunctionName=fn,
        InvocationType="Event",
...
async def post_group_approval_to_teams_channel(  # noqa: PLR0912
    ...
    """Build group approval card, post/update in Teams, persist presentation, schedule follow-ups."""
    c = deps.cfg
    ...
    if not teams_conversation_id:
        log.warning("Deferred group post: empty teams_conversation_id for %s", elevator_id)
        return

    rec = request_store.get_access_request(elevator_id)
    if rec is None or rec.kind != ElevatorRequestKind.group:
        log.warning("Deferred group post: missing or non-group request %s", elevator_id)
        return
    ...
    try:
        sso_instance = sso_mod.describe_sso_instance(sso_client, c.sso_instance_arn)
        sso_group = sso_mod.describe_group(sso_instance.identity_store_id, str(rec.group_id or ""), identity_store_client)
    except Exception:
    ...
    try:
        ...
        if launcher_id:
            try:
                ...
                log.exception("In-place group approval card update failed, posting new message: %s", e)
        ...
        if act_id and show_buttons and decision.reason == access_control.DecisionReason.RequiresApproval and decision.approvers:
            try:
                ...
                log.exception("Failed to @mention approvers in Teams (group): %s", e)
        ...
        if act_id and decision.grant and auto_status:
            try:
                ...
                log.exception("Failed to post auto-grant Teams thread messages (group): %s", e)
        ...
        if show_buttons and act_id:
            ...
            log.warning(
                "Teams group request: no activity id after post; cannot persist presentation or schedule discard/renotification: %s",
                elevator_id,
            )
    except Exception as e:
        log.exception("Failed to post approval card to Teams channel (group deferred): %s", e)
...
async def run_post_group_approval_worker(event: dict[str, Any]) -> dict[str, Any]:
    """Second Lambda entry: HMAC + post group card + schedule (invoked with InvocationType=Event)."""
    eid = str(event.get("elevator_id", ""))
    rname = str(event.get("requester_display_name", ""))
    email = str(event.get("requester_email", ""))
    teams_conversation_id = str(event.get("teams_conversation_id", ""))
    ...
    if not eid or not email or not teams_conversation_id or not verify_group_approval_post(eid, email, thread, sig):
        log.warning("Invalid or unsigned teams_post_group_approval event")
        return {"statusCode": 403, "body": "forbidden"}
    ...
    configure_teams_dependencies(deps)
    await get_teams_app()
    await post_group_approval_to_teams_channel(deps, eid, rname, email, thread)
    return {"statusCode": 200, "body": "ok"}
