"""Slack Bolt app for the access requester — import only when ``chat_platform`` is ``slack``."""
...
def get_slack_app(ctx: RequesterContext) -> App:
    """Build and cache the Slack :class:`App` once per cold start."""
    global _slack_app
    if _slack_app is None:
        _slack_app = _build_slack_app(ctx)
    return _slack_app


def _build_slack_app(ctx: RequesterContext) -> App:
    cfg = ctx.cfg
    ...
    def acknowledge_request(ack: Ack) -> None:
        ack()
    ...
    def handle_duration_picker_action(ack: Ack) -> None:
        ack()
