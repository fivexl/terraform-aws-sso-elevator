...
def verify_slack_request(headers, signing_secret, body, age=60):
    ...
    if 'X-Slack-Request-Timestamp' in headers:
        ...
        raise Exception('Request does not have X-Slack-Request-Timestamp or x-slack-request-timestamp')
    if not timestamp.isdigit():
        raise Exception('Value of X-Slack-Request-Timestamp does not appear to be a digit')
