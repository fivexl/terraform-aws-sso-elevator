...
# Teams routing is defined in :mod:`teams_handlers` and invoked via :func:`handle_teams_event` below.


async def handle_teams_event(event: dict, context) -> dict:  # noqa: ANN001, ARG001
    """Process Azure Bot / Teams activity via microsoft-teams-apps (JWT + routes)."""
    return await teams_runtime.process_teams_lambda_event(event)
