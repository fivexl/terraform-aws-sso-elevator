...
def lambda_handler(event, context):
    ...
    # Parse payload
    payload = dict(urlparse.parse_qsl(base64.b64decode(str(event['body'])).decode('ascii')))  # data comes b64 and also urlencoded name=value& pairs
...
def main(payload, slack_channel_id, token, dynamodb_table_name):
    ...
    if payload['type'] == 'shortcut':
        ...
        inital_form = prepare_slack_initial_form(trigger_id, config_lookup('permission_sets'), config_lookup('accounts'))
    ...
    # Form submitted
        ...
        if requires_approval:
            approvers_slack_ids = [f'<@{config_lookup("users", "email", approver, "slack_id")}>' for approver in approvers]
            ...
            post_slack_message('/api/chat.postMessage', approval_request_notification_message, token)
    ...
    # someone pressed a button
        ...
        if payload['actions'][0]['value'] != 'approve' and payload['actions'][0]['value'] != 'deny':
            ...
            return {'statusCode': 500}
        account_id = find_value_in_content_block(payload['message']['blocks'], 'AccountId')
        approvers = config_lookup('accounts', 'id', account_id, 'approvers')
        approvers_slack_ids = [config_lookup('users', 'email', approver, 'slack_id') for approver in approvers]
        requires_approval = True if len(approvers) > 0 else False
        approvers = [user["slack_id"] for user in config_lookup('users') if user['can_approve']]
        if payload['actions'][0]['value'] == 'approve' and requires_approval and payload['user']['id'] not in approvers_slack_ids:
        ...
        if payload['actions'][0]['value'] == 'approve':
            ...
            account_id = find_value_in_content_block(payload['message']['blocks'], 'AccountId')
            role_name  = find_value_in_content_block(payload['message']['blocks'], 'Role name')
            # slack id will come with <@{requester_slack_id}> so we need to clean it
            requester_slack_id = find_value_in_content_block(payload['message']['blocks'], 'Requester')
            ...
            audit_entry = {
                ...
                'requester_email': config_lookup('users', 'slack_id', requester_slack_id_clean, 'email'),
                'request_id': request_id,
                'approver_slack_id': payload['user']['id'],
                'approver_email': config_lookup('users', 'slack_id', payload['user']['id'], 'email'),
                'operation_type': 'grant'
            }
            
            response = log_operation_to_dynamodb(logger, dynamodb_table_name, audit_entry)
