...
| <a name="input_cli_expected_account_id"></a> [cli\_expected\_account\_id](#input\_cli\_expected\_account\_id) | AWS account ID that a CLI caller's verified IAM ARN must belong to for the request to be accepted. If not provided, defaults to the account this module is deployed into. | `string` | `""` | no |
...
| <a name="output_requester_api_endpoint_url_cli"></a> [requester\_api\_endpoint\_url\_cli](#output\_requester\_api\_endpoint\_url\_cli) | The full URL for the CLI's access-request route. Pass this to `elevate configure --endpoint` (or set as ELEVATE\_ENDPOINT). |
