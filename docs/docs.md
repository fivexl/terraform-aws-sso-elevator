...
3. Use SSO Elevator’s **group access** flow to request access to this `ManagementAccountAccess` group (Slack: **`group-access`** global shortcut; Teams: **`/group-access`**). This will add you to the group and grant you access to the management account (without directly using a permission set from the delegated admin account).
